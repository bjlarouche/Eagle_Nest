...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        ...
        self.parseClassName = @"Organization";
...
- (PFQuery *)queryForTable {
    PFQuery *query1 = [PFQuery queryWithClassName:@"Organization"];
    
    //Check for search
    if (_searchBar.text.length > 0) {
        [query1 whereKey:@"name" matchesRegex:[NSString stringWithFormat:@"(?i)%@", _searchBar.text]];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ((lastRowIndex == 0) || (indexPath <= pathToLastRow)) {
        ...
        OrgProfileController *controller = [storyboard instantiateViewControllerWithIdentifier:@"OrgProfileController"];
        
        controller.organizationObject = cell.object;
        controller.objectId = cell.objectId;
