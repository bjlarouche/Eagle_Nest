...
@interface MessagesController () <JSQMessagesViewAccessoryButtonDelegate>{
    PFObject *channel;
}
@end
...
- (void)viewDidLoad
{
    ...
    UIColor *redcolor = [UIColor colorWithRed:203/255 green:45/255 blue:62/255 alpha:1.0];
    [[UINavigationBar appearance] setTintColor:redcolor];
    [[UIToolbar appearance] setTintColor:redcolor];
    [[UIBarButtonItem appearance] setTintColor:redcolor];
     ...
     */
    self.messageData = [[MessageModelData alloc] init];
    [self loadAvatars];
...
//    }
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage jsq_defaultTypingIndicatorImage] style:UIBarButtonItemStylePlain target:self action:@selector(dotsPressed:)];
...
//    JSQMessagesAvatarImage *jsqImage = [avatarFactory avatarImageWithUserInitials:@"JSQ" backgroundColor:[UIColor colorWithWhite:0.85f alpha:1.0f] textColor:[UIColor colorWithWhite:0.60f alpha:1.0f] font:[UIFont systemFontOfSize:14.0f]];
    ...
    PFQuery *membersInChannel = [PFUser query];
    [membersInChannel whereKey:@"channels" containsString:_channelObject.objectId];
    NSArray *objects = [membersInChannel findObjects];
    for (PFUser *user in objects) {
        PFFile *imageFile = user[@"profileImage"];
        NSData *data = [imageFile getData];
        
        UIImage *avatar = [JSQMessagesAvatarImageFactory circularAvatarImage:[UIImage imageWithData:data] withDiameter:kJSQMessagesCollectionViewAvatarSizeDefault];
        UIImage *highlightedAvatar = [JSQMessagesAvatarImageFactory circularAvatarHighlightedImage:avatar withDiameter:kJSQMessagesCollectionViewAvatarSizeDefault];
...
- (void)dotsPressed:(UIBarButtonItem *)sender {
    // Dots pressed
}
                      ...
                      date:(NSDate *)date
{
    ...
    messageObject[@"channel"] = _channelObject.objectId;
    messageObject[@"sender"] = senderId;
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    switch (buttonIndex) {
        ...
        case 1:
        {
            ...
            [self.messageData addLocationMediaMessageCompletion:^{
                [weakView reloadData];
            }];
        }
            break;
        ...
        case 2:
            [self.messageData addVideoMediaMessage];
            break;
            
        case 3:
            [self.messageData addVideoMediaMessageWithThumbnail];
            break;
            
        case 4:
            [self.messageData addAudioMediaMessage];
            break;
    }
...
- (NSString *)senderDisplayName {
    ...
    return [[NSString alloc] initWithFormat:@"%@ %@", currentUser[@"firstname"], currentUser[@"lastname"]];
}
...
#pragma mark -- LiveQuery Recieve messages

-(void)setupLiveQuery {
    self.liveQueryClient = [[PFLiveQueryClient alloc] initWithServer:@"wss://bcapp.back4app.io" applicationId:@"Kya2WjQrGgZEl8xoJ8P81q4gUmyJCDoLufl7AKII" clientKey:@"H5Ae6c5Vvzl7xzZ7p76fZP1c9mt5fiqRsMnFtqZy"];
    
    self.msgQuery = [PFQuery queryWithClassName:@"Message"];
    
    [self.msgQuery whereKey:@"channel" equalTo:_channelObject.objectId];
    ...
    [self.subscription addCreateHandler:^(PFQuery<PFObject *> *query, PFObject *object) {
        ...
        if (object != nil) {
            PFUser *sender = [PFQuery getUserObjectWithId:object[@"sender"]];
            
            JSQMessage *newMessage = [[JSQMessage alloc] initWithSenderId:sender.objectId
                                                     senderDisplayName:[[NSString alloc] initWithFormat:@"%@ %@", sender[@"firstname"], sender[@"lastname"]]
                                                                  date:[NSDate date]
                                                                  text:object[@"text"]];
            
            
            [_messageData.messages addObject:newMessage];
